    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                feat = backbone(images, prompt_model)
...
if __name__ == "__main__":
    ...
    prompt_model = PromptResNet().cuda()
    args.patch_size = 16
    ...
    classifier = nn.Sequential(backbone, projector).to(device)
    model = nn.Sequential(prompter, classifier).to(device)
    model.load_state_dict(
        torch.load('/pretrain_model/scars.pt', map_location='cpu'),
        strict=False)
    backbone.to(device)
    projector.to(device)
    ...
    # TRAIN
    # ----------------------
    train(model, backbone, projector, train_loader, test_loader_labelled, test_loader_unlabelled, args, prompt_model)
